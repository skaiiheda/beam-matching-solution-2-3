...
def create_envelope_plot(
    twiss_in: TwissParamsXY, config: BeamlineConfig, quads: QuadrupoleSettings
) -> go.Figure:
    """
    ...
    fig.update_layout(
        title=dict(
            text="<b>Огибающая пучка (σ = √(ε·β))</b>",
            x=0.5,
            xanchor="center",
            font=dict(size=16),
        ),
        xaxis=dict(title="s (м)", showgrid=True, gridcolor="#f1f5f9", zeroline=False),
        yaxis=dict(
            title="σ (мкм)",
