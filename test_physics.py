...
def test_twiss_propagation():
    ...
    beta2_check = (
        m11 * m11 * twiss.beta - 2 * m11 * m12 * twiss.alpha + m12 * m12 * gamma
    )
    alpha2_check = (
        -m11 * m21 * twiss.beta
...
def test_matching_error():
    ...
    twiss_out = TwissParamsXY(
        x=TwissParams(beta=7.9, alpha=0.01), y=TwissParams(beta=4.1, alpha=-0.01)
    )

    error = calculate_matching_error(twiss_out, DEFAULT_TWISS_TARGET)
    print(
        f"Target Twiss: βx*={DEFAULT_TWISS_TARGET.x.beta}, αx*={DEFAULT_TWISS_TARGET.x.alpha}"
    )
    print(
        f"             βy*={DEFAULT_TWISS_TARGET.y.beta}, αy*={DEFAULT_TWISS_TARGET.y.alpha}"
    )
...
def test_envelope():
    ...
    twiss_out, history = propagate_through_beamline_x(
        TwissParams(beta=5.0, alpha=-0.5), config, quads
    )
    ...
    for i in range(3):
        ...
        print(
            f"  {i + 1}. s={s:.3f} m, σ={sigma * 1e6:.2f} μm, σ⁻={sigma_neg * 1e6:.2f} μm"
        )
...
def test_utils():
    ...
    for value, decimals, expected in test_cases:
        ...
        print(
            f"format_number({value}, {decimals}) = '{result}' (expected: '{expected}')"
        )
    ...
    for actual, target, expected in error_cases:
        ...
        print(
            f"percent_error({actual}, {target}) = {result:.2f}% (expected: {expected:.2f}%)"
        )
