...
from beam_matching import (
    ...
    DEFAULT_CONFIG, DEFAULT_TWISS_IN, DEFAULT_TWISS_TARGET,
    propagate_through_beamline_x, propagate_through_beamline_y,
    optimize_quadrupoles, calculate_matching_error
)
...
def optimize_quads():
    ...
    with st.spinner('Оптимизация квадруполей...'):
        ...
        st.session_state.quads = result['quads']
        st.success(f'Оптимизировано! Финальная ошибка: {result["error"]:.8e} (итераций: {result["iterations"]})')
...
def parameters_panel() -> None:
    ...
    twiss_out_x, _ = propagate_through_beamline_x(
        st.session_state.twiss_in.x,
        st.session_state.config,
        st.session_state.quads
    )
    twiss_out_y, _ = propagate_through_beamline_y(
        st.session_state.twiss_in.y,
        st.session_state.config,
        st.session_state.quads
    )
...
def main():
    ...
    with col2:
        ...
        with col_s1:
            if stats['is_matched']:
                ...
                st.warning(f"✗ Не согласовано: Средняя ошибка {stats['avg_error']:.2f}%")
        with col_s2:
    ...
    with col1:
        ...
        st.caption("Эллипсы представляют контур эмиттанса пучка: γx² + 2αxx' + βx'² = ε")
    ...
    with col2:
        ...
        with col_e1:
            st.metric("Эмиттанс X", f"{st.session_state.config.emit_x * 1e9:.1f} нм·рад")
        with col_e2:
            st.metric("Эмиттанс Y", f"{st.session_state.config.emit_y * 1e9:.1f} нм·рад")
    
    # Footer
    st.markdown("---")
    col_f1, col_f2, col_f3 = st.columns(3)
    with col_f1:
        st.caption("Матрицы переноса: Дрейф [[1, L], [0, 1]] | Квадруполь [[1, 0], [-k, 1]]")
    with col_f2:
